...
Now that we have our `RemarkCommand` ready to be executed, we need to update `IntelliJournalParser#parseCommand()` to recognize the `remark` keyword. Add the new command to the `switch` block by creating a new `case` that returns a new instance of `RemarkCommand`.
...
:information_source: Don’t forget to update `IntelliJournalParser` to use our new `RemarkCommandParser`!
