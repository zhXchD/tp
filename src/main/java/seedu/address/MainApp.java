...
public class MainApp extends Application {
     ...
     * address book and {@code userPrefs}. <br>The data from the sample address
     * book will be used instead if {@code storage}'s address book is not found,
     * or an empty address book will be used instead if errors occur when
    ...
    private Model initModelManager(Storage storage, ReadOnlyUserPrefs userPrefs) {
        ...
        try {
            ...
            if (addressBookOptional.isEmpty()) {
                logger.info("AddressBook data file not found. Will be "
                        + "starting with a sample AddressBook");
            }
            initialAddressBook = addressBookOptional
                    .orElseGet(SampleDataUtil::getSampleAddressBook);
        } catch (DataConversionException e) {
            logger.warning("AddressBook data file not in the correct "
                    + "format. Will be starting with an empty AddressBook");
            initialAddressBook = new AddressBook();
        } catch (IOException e) {
            logger.warning("Problem while reading from the AddressBook "
                    + "file. Will be starting with an empty AddressBook");
            initialAddressBook = new AddressBook();
        }
        ...
        try {
            ...
            initialJournal = journalOptional
                    .orElseGet(SampleDataUtil::getSampleJournal);
        } catch (DataConversionException e) {
            logger.warning("Journal data file not in the correct "
                    + "format. Will be starting with an empty Journal");
            initialJournal = new Journal();
        } catch (IOException e) {
            logger.warning("Problem while reading from the Journal "
                    + "file. Will be starting with an empty Journal");
            initialJournal = new Journal();
        }
     ...
     * The default file path {@code Config#DEFAULT_CONFIG_FILE} will be used
     * instead if {@code configFilePath} is null.
     */
    ...
    /**
     * Returns a {@code UserPrefs} using the file at {@code storage}'s user
     * prefs file path, or a new {@code UserPrefs} with default configuration
     * if errors occur when reading from the file.
     */
